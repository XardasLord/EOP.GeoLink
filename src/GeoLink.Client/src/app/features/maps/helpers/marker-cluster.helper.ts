...
export class MarkerClusterHelper {
  public static getCssClassForSuperclusterGroup(childMarkerDevicesData: Supercluster.PointFeature<any>[]): string {
    // TODO: We can change 'any' to MapItemModel and operate on its properties
    let css = '';

    const goodStatuses = childMarkerDevicesData.filter(
      x => (JSON.parse(x.properties.deviceData) as MapItemModel).status === 2
    ).length;

    const warningStatuses = childMarkerDevicesData.filter(
      x => (JSON.parse(x.properties.deviceData) as MapItemModel).status === 1
    ).length;

    const badStatuses = childMarkerDevicesData.filter(
      x => (JSON.parse(x.properties.deviceData) as MapItemModel).status === 0
    ).length;
